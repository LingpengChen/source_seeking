import numpy as np
from controller.basis import Basis
from controller.barrier import Barrier
from controller.replay_buffer import ReplayBuffer
...
#         self.lamk  = 1.0/(np.linalg.norm(self.basis.k, axis=1) + 1)**(3.0/2.0)
        ...
        if weights is None:
            ...
            weights = {'R' : weights*np.eye(self.model.action_space.shape[0])}
        self.Rinv = np.linalg.inv(weights['R'])

        self._phik = None
        self._ck = None
        self._consensus_ck = None
        self.Erg_metric = 0
    ...
    def __call__(self, state):
        ...
        # x would be the state set by the def control
        ...
        N = len(pred_traj)
        self._ck = np.sum([self.basis.fk(xt) for xt in pred_traj], axis=0) / N

        ck = None
        if self._consensus_ck is not None:
            ck = self._consensus_ck
        else:
            ck = self._ck
