...
from GreedyBO_robot import Robot
...
def print_progress_bar(index, iteration):
    ...
    sys.stdout.write(f"\rIteration {iteration}")  # 写入新内容
    sys.stdout.flush()
...
def experiment():
    ...
    for iteration in range(1000):
        ...
        for i in range(robo_num):
            ...
            # print(f"ROBOT{i} have {len(sources)} sources.")
            peaks += sources
            UCB_list += ucb_value 
        
        
        ## 3. communication founded sources
        found_source = []
        for i in range(robo_num):
        ...
        for i in range(robo_num):
            ...
            setpts, target = Robots[i].get_nextpts() 
    ...
    ## Visualize
       ## Visualize
        ...
        debug_print = DEBUG and (iteration == 0 or (iteration >= 10 and iteration % 5 == 0))
        ...
        if debug_print or end:
            ...
            if True:   
                ...
                if (len(peaks)!=0):
                    ...
                    for i, (x, y) in enumerate(zip(x_coords, y_coords)):
                        axs[0].text(x, y, f"{UCB_list[i]:.2f}", c='green', fontsize=6, ha='right', va='top')
...
if __name__ == '__main__':
    ...
    print("Start experiment case", experiment_case)
    ...
    save_img_path = "/home/clp/catkin_ws/src/source_seeking/record/7sources/GreedyBO/experiment_case_" + str(experiment_case) + ".png"
    save_rmse_path = "/home/clp/catkin_ws/src/source_seeking/record/7sources/GreedyBO/experiment_case_" + str(experiment_case) + ".txt"
