...
from environment_and_measurement import f, sampling # sampling function is just f with noise     
from environment_and_measurement import SOURCES_case, SOURCE_VALUE    # while SOURCES_case and SOURCE_VALUE are variables
...
def experiment():
    ...
    for iteration in range(1000):
        ...
        if True:
            ...
            for i in range(robo_num):
                temp_dictionary_X, temp_dictionary_y = Robots[i].voronoi_update(neighbour_list[i], robot_locations) # 交换给neighbour的样本字典
...
if __name__ == '__main__':
    ...
    SOURCES = SOURCES_case[args.source_index]

    SOURCE_set = {tuple(item) for item in SOURCES}
    FIELD_SIZE_X = 10
    FIELD_SIZE_Y = 10
    x_min = (0, 0)
    ...
    if True:
        ...
        for index in range(robo_num):
            ...
            instance = Robot(start_position=robot_locations[index], index=index, test_resolution=[50,50], field_size=[10,10]) # resolution is 50 * 50
