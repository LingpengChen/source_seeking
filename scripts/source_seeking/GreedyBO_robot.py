...
from environment.environment_and_measurement_5 import Environment, DEBUG
from environment.environment_and_measurement_5 import CAM_FOV, SRC_MUT_D_THRESHOLD, LCB_THRESHOLD, STUCK_PTS_THRESHOLD 
...
def kernel_initial(
        ...
        ell_initial=1.0,        # length scale
        σn_initial=0.1,         # noise level
        σf_bounds=(0.1, 10.0),  # amplitude bounds
        length_bounds=(0.1, 5.0),    # length scale bounds
        noise_bounds=(1e-10, 1e+5)   # noise level bounds
    ):
        return σf_initial**2 * RBF(
            length_scale=ell_initial, 
            length_scale_bounds=length_bounds
        ) + WhiteKernel(
            noise_level=σn_initial, 
...
class Robot(object): # python (x,y) therefore col index first, row next
    ...
    def get_nextpts(self):
        ctrl = None
        target = None    
           
        selfucb = self.ucb * self.responsible_region
        flat_ucb = selfucb.T.flatten()
        i = 0

        # First find the argmax that has not been visited within the cell         
        while True:
            i+=1
            ith_max_index = np.argsort(flat_ucb)[-i]
            ith_max = flat_ucb[ith_max_index]
            ith_max_index_2d = np.unravel_index(ith_max_index, selfucb.shape)
            ith_max_coordinate = np.array(ith_max_index_2d) * (self.field_size[0] / self.test_resolution[0])
            
            if any(np.linalg.norm(np.array(ith_max_coordinate) - np.array(peak)) < SRC_MUT_D_THRESHOLD for peak in self.visited_peaks_cord) or \
            any(np.linalg.norm(np.array(ith_max_coordinate) - np.array(stuck)) < STUCK_PTS_THRESHOLD for stuck in self.stuck_points):
                continue
        ...
        # remove the peak that get stuck
        for i, peak in enumerate(self.peaks_cord):
        ...
        peaks_cord = [peak for i, peak in enumerate(self.peaks_cord) if i not in indices_to_remove] + [ith_max_coordinate]
        peaks_UCB = [ucb_value for i, ucb_value in enumerate(self.peaks_UCB) if i not in indices_to_remove] + [ith_max]
        
        # select argmax (among UCB peaks and maximum place (may at boundry))
    ...
    def estimate_source(self, ucb_coeff):
        peaks = find_peak(self.ucb, strict=True)
